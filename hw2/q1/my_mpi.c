...
void client(char* hostname,int portno,MPI_Comm comm,int bytes,int *msg)
{
	...
	//printf("entered client ---%d,%s\n",portno,hostname);
    	...
    	if (server == NULL) {
        	//f//printf(stderr,"ERROR, no such host\n");
        	exit(0);
    	}
	...
	{
		int *buff;
    		buff=(int *)malloc(sizeof(int)*2);
    		buff[0]=comm.rank;
    		int i,size;
		//siize=bytes/sizeof(int);
		buff[1]=msg[0];
	...
	//		buff[i+1]=msg[i];
	//	}
    		n = write(sockfd,buff,(2*(sizeof(int))));
	...
	while(flag==false)
	{
		...
		if(portno!=30014)
		{
			...
			{           
     				...
     				//n=0;
				int * buff_glob;
				buff_glob=(int *)malloc(2*sizeof(int));
				int n1=0;
				int size=2*sizeof(int);
				while(n1< size)
				{
				n = read(newsockfd,buff_glob,size);
				if(n>0)
				{
					n1+=n;
					buff_glob+=n;
		...
		else
		{
			...
			if(barrier==true)
			{
				...
				while(n1<size)
				{
        			n = read(newsockfd,buff_glob,(2*sizeof(int)));
				if(n>0)
				{
...
{
	if(MPI_COMM_WORLD.rank<(MPI_COMM_WORLD.numproc/2))
        {	
...
int main(int argc,char *argv[])
{
	...
	bufferSend=(int *)malloc(1*1024*1024);
	bufferRecv=(int *)malloc(1*1024*1024);
	*(bufferSend+127)=32;
